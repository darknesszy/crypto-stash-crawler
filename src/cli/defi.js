import { readBalance as binanceBalance, readExchangeRate as binanceExchangeRate } from '../plugins/defi/binance'
...
export const runAll = (outputFn, data) => Promise.all([
...
const hasAllParams = options => hasBalanceParams(options)
    && hasExchangeRateParams(options)

const hasBalanceParams = options => (options.p || options.provider)
    && (options.a || options.apikey) && (options.s || options.secret)

const hasExchangeRateParams = options => (options.p || options.provider)
...
const taskFn = (options, params, outputFn) => taskFnMap[task(options)].runFn(outputFn, params)
